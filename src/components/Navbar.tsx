...
function NavLink({ href, onClick, children }: NavLinkProps) {
  return (
    <span
      onClick={onClick}
      className="text-gray-700 hover:text-gray-900 transition-colors cursor-pointer whitespace-nowrap"
    >
      {children}
    </span>
  );
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex-shrink-0">
            <a
              ...
              className="text-lg md:text-xl lg:text-2xl font-bold text-gray-900 cursor-pointer whitespace-nowrap"
            >
              Supratip | Developer
            </a>
          </div>
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-md text-gray-700 hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
          >
        ...
        {isOpen && (
          <div className="lg:hidden">
            ...
            <div className="fixed inset-x-0 top-16 bg-white shadow-lg">
              <div className="pt-2 pb-3 space-y-1 px-4 max-h-[calc(100vh-4rem)] overflow-y-auto">
                {NAV_ITEMS.map((item) => (
                  <NavLink
                    ...
                    onClick={() => scrollToSection(item.href)}
                  >
                    <div className="block px-3 py-2 rounded-md text-base font-medium hover:bg-gray-50">
                      {item.label}
                    </div>
                  </NavLink>
                ))}
                <div className="flex items-center space-x-4 px-3 py-4 border-t border-gray-200 mt-4">
                  {SOCIAL_LINKS.map((link) => (
                    <SocialIcon key={link.href} {...link} />
                  ))}
                </div>
